...
/**
	...
	wet_floor
		...
		id = "wet_floor"
		unique = TRUE
		var/datum/reagent/turf_fluid = null
		var/fluid_name = null
		var/behavior_type = "wet"
		var/turf/turf = null
		...
		onAdd(optional)
			...
			src.fluid_name = src.turf_fluid.id
			...
			switch (src.fluid_name) // custom behavior
				if ("spaceglue")
			...
			turf.AddOverlays(overlay, "wet_overlay")
			dry_out()

		onUpdate(timePassed)
			var/obj/fluid/F = locate(/obj/fluid) in owner.contents
			src.cloth = locate(/obj/item/clothing/under/towel) in owner.contents
			src.coat = locate(/obj/item/clothing/suit) in owner.contents
			var/mob/living/carbon/human/H = locate(/mob/living/carbon/human) in owner.contents
			if (F)
				src.fluid_name = F.group.master_reagent_id
				src.turf_fluid = F
			else
				src.turf_fluid = null
		...
		proc/wet_behavior(var/mob/living/carbon/human/target = null)
			if (src.chump && !ON_COOLDOWN(target, "free_kick_on_\ref[src]", target.trample_cooldown)) // don't kick your chump!
				boutput(H, SPAN_NOTICE("You walk over [src.chump.name]'s body!"))
				...
				var/list/clothes = src.chump.get_all_items_on_mob()
				for (var/obj/item/clothing/C in clothes)
					C.add_stain("[fluid_name]-stained")
				return
			else if (src.cloth)
				src.cloth.add_stain("[src.fluid_name]-stained")
				return
			else if (src.coat)
				src.coat.add_stain("[src.fluid_name]-stained")
				return

			switch (src.fluid_name)
				if ("spaceglue")
					src.behavior_type = "slow"
				...
				if ("superlube", "invislube")
					if (src.silent)
						playsound(target.loc, 'sound/misc/slip.ogg', 50, TRUE, -3) // invislube is stealthy
					target.remove_pulling()
			...
			if (target && target.hasStatus("resting"))
				chump = target
				return
